...
public class SecurityConfiguration {
	...
	public SecurityFilterChain config(HttpSecurity httpSecurity) throws Exception {
		return httpSecurity.csrf(csrf -> csrf.disable())
				.authorizeHttpRequests(req -> req.requestMatchers("/saveData","/authenticate").permitAll().requestMatchers("/admin/**")
						.hasRole("ADMIN").anyRequest().authenticated())
				.formLogin() // Use the default Spring Security login page
				.and().logout(logout -> logout.permitAll() // Allow everyone to see the logout page
				).build();
